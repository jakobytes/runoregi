...
from data.search import search_translated, search_types, search_verses, search_smd
    ...
    if args['q'] is None:
        ...
        with config.get_db() as db:
            ...
            r_inline = [(nro, pos, text) \
                        for (nro, pos, vtype, text) in r_verses_and_inline \
                        if vtype != 'V']
            r_translated = search_translated(db, args['q'])
        data = { 'r_verses': r_verses, 'r_types': r_types,
                 'r_inline': r_inline, 'r_translated': r_translated,
                 'r_smd': r_smd,
